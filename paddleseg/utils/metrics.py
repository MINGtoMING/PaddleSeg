...
def calculate_area(pred, label, num_classes, ignore_index=255, use_multilabel=False):
    ...
    Args:
        ...
        use_multilabel(bool, optional): whether open multi-label mode
    ...
    if not use_multilabel:
        ...
        intersect = paddle.logical_and(pred.astype('bool'), label.astype('bool'))
        intersect_area = intersect.sum([0, 2, 3]).astype('int64')
